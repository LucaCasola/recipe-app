...
export const DesktopSidebar = ({
  ...
  const [ toggle ] = useAtom(sidebarToggleAtom);
  ...
  return (
    <>
      ...
      <motion.div
        className={cn(
          "h-full px-4 py-4 hidden md:flex md:flex-col bg-primary w-[100px] flex-shrink-0",
          className
        )}
        animate={{
          width: animate ? (open ? "230px" : "75px") : "230px",
        }}
        onMouseEnter={() => !toggle && setOpen(true)}
        onMouseLeave={() => !toggle && setOpen(false)}
        {...props}
      >
...
export const SidebarLink = ({
  ...
  const isActive = pathname.startsWith(link.href); // Check if the current path matches the link's path
